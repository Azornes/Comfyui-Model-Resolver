...
function isInsideWebRoot(targetPath) {
  const pathFromWebRoot = relative(WEB_ROOT, targetPath);
  return Boolean(
    pathFromWebRoot === ''
      || (!pathFromWebRoot.startsWith(`..${sep}`)
        && pathFromWebRoot !== '..'
        && !isAbsolute(pathFromWebRoot))
  );
}

async function resolveLocalModule(importerPath, specifier) {
  const targetPath = resolve(dirname(importerPath), specifier);
  if (!isInsideWebRoot(targetPath)) {
    // Imports outside web/ are ComfyUI-provided modules such as scripts/app.js.
    return null;
  }
...
test('all local frontend module imports resolve to files', async () => {
  const sourceFiles = await collectJavaScriptFiles(WEB_ROOT);
  ...
  for (const importerPath of sourceFiles) {
    ...
    for (const specifier of extractModuleSpecifiers(source)) {
      ...
      missingImports.push({
        importer: relative(WEB_ROOT, importerPath),
        specifier,
        expected: relative(WEB_ROOT, resolvedPath),
      });
    }
  }
